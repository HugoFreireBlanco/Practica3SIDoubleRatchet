...
# TENGO QUE VER COMO ARREGLAR QUE EL ROOT KEY DE ENVIO NO ES EL DE RECEPCION 
...
def main():
    ...
    safe_print("━" * 70)
    safe_print("        🔐 SERVIDOR - DOUBLE RATCHET ENCRYPTION PROTOCOL")
    safe_print("━" * 70)
    ...
    safe_print("⏳ Esperando conexión del cliente...\n")
    conn, addr = server.accept()
    safe_print("✓ Cliente conectado: " + str(addr) + "\n")
    ...
    received_public_key = receive_public_key(conn)
    ...
    while True:
        ...
        conn.send(public_key_bytes + ciphertext)
